...
class TestArticleViews:
    def test_get_articles_by_author(self, testapp, user):
        ...
        resp = testapp.post_json(
            url_for("user.login_user"), {"user": {"email": user.email, "password": "myprecious"}}
        )

        token = str(resp.json["user"]["token"])
        for _ in range(2):
            testapp.post_json(
                url_for("articles.make_article"),
                {
                    "article": {
    ...
    def test_make_comment_correct_schema(self, testapp, user):
        ...
        resp = testapp.post_json(
            ...
            {"comment": {"createdAt": datetime.now().isoformat(), "body": "You have to believe"}},
            headers={"Authorization": "Token {}".format(token)},
        )
