...
def _register_user(testapp, **kwargs):
    return testapp.post_json(
        ...
        {"user": {"username": "mo", "email": "mo@mo.mo", "password": "momo"}},
        **kwargs,
    )
...
class TestAuthenticate:
    ...
    def test_user_login(self, testapp):
        ...
        resp = testapp.post_json(
            url_for("user.login_user"), {"user": {"email": "mo@mo.mo", "password": "momo"}}
        )
    ...
    def test_get_user(self, testapp):
        ...
        resp = testapp.get(
            url_for("user.get_user"), headers={"Authorization": "Token {}".format(token)}
        )
        assert resp.json["user"]["email"] == "mo@mo.mo"
    ...
    def test_update_user(self, testapp):
        ...
        resp = testapp.put_json(
            url_for("user.update_user"),
            {"user": {"email": "meh@mo.mo", "bio": "I'm a simple man", "password": "hmm"}},
