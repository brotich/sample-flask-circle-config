...
def _register_user(testapp, **kwargs):
    return testapp.post_json(
        ...
        {"user": {"email": "foo@bar.com", "username": "foobar", "password": "myprecious"}},
        **kwargs,
    )
...
class TestProfile:
    ...
    def test_follow_user(self, testapp, user):
        ...
        resp = testapp.post(
            url_for("profiles.follow_user", username=user.username),
    ...
    def test_unfollow_user(self, testapp, user):
        ...
        token = str(resp.json["user"]["token"])
        resp = testapp.delete(
            url_for("profiles.unfollow_user", username=user.username),
            headers={"Authorization": "Token {}".format(token)},
        )
        assert not resp.json["profile"]["following"]
